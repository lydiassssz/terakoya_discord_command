  ...
  if (body.type === 2) {
    ...
    (async () => {
      if (commandName === "hello") {
        ...
        // 権限剥奪
        const success = await modifyUserChannelPermission(
          channelId,
          userId,
          DISCORD_BOT_TOKEN,
          { allow: 0, deny: VIEW_CHANNEL }
        );
        // ログ送信
        await sendLogMessage(body, DISCORD_BOT_TOKEN, BOT_LOG_CHANNEL_ID);
        ...
        if (!success) {
          ...
          await followUpMessage(
            body,
            DISCORD_BOT_TOKEN,
            "Failed to remove access. Check bot permissions."
          );
          return;
        }
        ...
        if (dmChannelId) {
          ...
          await sendDM(
            dmChannelId,
            "Your access to the channel was removed. Click 'Revert' to restore it.",
            customId,
            DISCORD_BOT_TOKEN
          );
        } else {
        ...
        await followUpMessage(
          body,
          DISCORD_BOT_TOKEN,
          "Your access to this channel has been removed. Check your DMs for a revert option."
        );
      } else {
        // 未対応コマンド
        await sendLogMessage(body, DISCORD_BOT_TOKEN, BOT_LOG_CHANNEL_ID);
  ...
  if (body.type === 3) {
    ...
    (async () => {
      if (customId.startsWith("revert_access-")) {
        ...
        // 権限復元
        const revertSuccess = await modifyUserChannelPermission(
          channelId,
          userId,
          DISCORD_BOT_TOKEN,
          { allow: 0, deny: 0 }
        ...
        if (dmChannelId) {
          if (!revertSuccess) {
            await sendDM(
              dmChannelId,
              "Failed to revert access. Check bot permissions.",
              null,
              DISCORD_BOT_TOKEN
            );
          } else {
...
async function modifyUserChannelPermission(
  channelId,
  userId,
  botToken,
  { allow, deny }
