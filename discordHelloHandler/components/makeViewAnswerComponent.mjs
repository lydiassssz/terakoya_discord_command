...
export function handleViewAnswerButton(body) {
    ...
    // すでに回答を見たかどうかのチェック
    // ここではサンプルとして「false=未確認固定」としているが、
    // 実際にはDB等で確認状況を判定する実装が必要
    const alreadyViewed = false; // checkIfUserAlreadyViewed(userId, messageId);

    if (alreadyViewed) {
        // 既に確認している場合はエラーメッセージをエフェメラル(本人にしか見えない)で返す
        return respondJSON({
            type: 4, // CHANNEL_MESSAGE_WITH_SOURCE
            data: {
                content: "すでにこの回答を確認済みです。",
                flags: 64, // 64: EPHEMERAL (本人のみ見えるメッセージ)
            },
        });
    } else {
        // 未確認の場合はトークン消費のメッセージを表示
        return respondJSON({
            type: 4, // CHANNEL_MESSAGE_WITH_SOURCE
            data: {
                content: "問題を解かずに回答を見るには100トークンが必要です。",
                flags: 64, // 64: EPHEMERAL (本人のみ見えるメッセージ)
            },
        });
    }
